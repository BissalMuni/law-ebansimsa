...
헌법 P1: 모든 AI 문장은 위키 근거를 추적 가능해야 하며, 근거 없는 내용은
NO_BASIS_TOKEN("기준에 없음")으로 명시한다 (FR-005a).
헌법 P4: 참고 조례는 복제 금지를 AI에 강제한다.
"""
...
# FR-005a — 입안·정비 기준에 근거가 없는 내용을 표시하는 토큰 (지어내기 금지)
...
def build_system_prompt(req: DraftRequest) -> str:
    ...
    - 근거 위키 섹션을 본문째 주입하고 citations로 추적 가능하게 한다 (P1)
    - 근거가 없으면 NO_BASIS_TOKEN으로 표기하도록 강제한다 (FR-005a)
    - 참고 조례가 있으면 "복사하지 말 것"을 강제한다 (P4)
    """
    parts: list[str] = [
    ...
    # FR-005a — 근거 없는 내용은 반드시 표시, 지어내기 금지
    parts.append(
