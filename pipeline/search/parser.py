"""기존 조례 원문을 조(條) 단위로 파싱한다 (T026).

개정 모드 로드용. 본문 저장 단위는 조 단위이고 항·호는 본문 마크업으로 둔다 (spec #1, FR-016).
기존 pipeline.review.article_splitter를 재사용해 복제를 피한다 (헌법 P4 정신).
"""
...
from pipeline.review.article_splitter import split_ordinance
from pipeline.review.models import DOCUMENT_ID

_ARTICLE_NO = re.compile(r"제(\d+)조")


class ParsedArticle:
    """조 단위 파싱 결과 (web의 OrdinanceSection으로 저장됨)"""
