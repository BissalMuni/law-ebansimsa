...
- [ ] [T008] `api/` FastAPI + uv 초기화 (`api/pyproject.toml`, `api/main.py`)
- [ ] [T009] [P] `api/` Pydantic 스키마 — Literal = data-model §4 허용값 (`api/schemas.py`)
...
- [ ] [T015] 단계 잠금 로직 — locked→…→confirmed, 우회 불가 (P2) (`web/src/server/stages.ts`)
- [ ] [T016] `api` `/draft/generate` SSE — pipeline/draft, 위키 근거 주입, 복제 금지(P4) (`api/routers/draft.py`, `pipeline/draft/`)
- [ ] [T017] Monaco 에디터 통합 + 조문 표시 (`web/src/components/Editor.tsx`)
- [ ] [T018] AI 채팅 패널(Secondary Sidebar) + 근거 토글 (P1) (`web/src/components/ChatPanel.tsx`)
- [ ] [T019] 조문 저장 — OrdinanceSection + Message(citations) + Snapshot (`web/src/server/sections.ts`)
- [ ] [T020] `api` `/validate/precise` — pipeline/review detailed, 1조문×1기준 강제 (P3) (`api/routers/validate.py`)
...
- [ ] [T026] `api` `/parse/ordinance` — 조 단위 파싱 (`api/routers/parse.py`, `pipeline/search/`)
...
- [ ] [T029] `api` `/search/ordinances` — 국가법령정보센터 OpenAPI (P4) (`api/routers/search.py`, `pipeline/search/`)
- [ ] [T030] 타 지자체 검색 모달 + AI 컨텍스트 첨부 (US7) (`web/src/components/ReferenceSearch.tsx`)
- [ ] [T031] `api` `/validate/inline` — Haiku 힌트 (`api/routers/validate.py`)
...
- [ ] [T035] `api` `/export` DOCX/PDF — pipeline/export, 온디맨드(blob 없음) (US9) (`api/routers/export.py`, `pipeline/export/`)
