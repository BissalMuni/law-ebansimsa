...
### P1. 모든 AI 작성 문장은 위키 출처를 추적·확인할 수 있어야 한다
AI가 생성한 조례 문장은 어느 입안·정비 기준(위키 섹션)을 근거로 했는지 항상 추적 가능해야 한다. (`Message.citations`) 근거 없는 생성은 "기준에 없음" 명시 없이는 허용하지 않으며, 입안·정비 기준에 근거가 없는 내용은 "기준에 없음"으로 표시한다.

### P2. 단계 잠금은 우회 불가하다
표준 8단계(`title`→`purpose`→`definition`→`scope`→`main`→`supplementary`→`review`→`finalize`, 키↔한글명 대조는 data-model §4.1)는 선형 잠금된다. 이전 단계가 `confirmed`가 아니면 다음 단계로 진입할 수 없다. 확정된 단계 수정 시 이후 단계는 `stale` 마크된다.
...
참고 조례를 AI 컨텍스트에 첨부할 때 "참고하되 그대로 복사하지 말 것"을 AI에 강제한다.
...
### P7. 진실원은 단일하며 파생물은 영속화하지 않는다
위키는 DB에 넣지 않는다 — 마크다운이 유일한 진실원이다. DiffHunk 등 파생 데이터는 렌더 시 계산하고 영속화하지 않는다.
...
**제약**: 위키 진실원·파생물 비영속 규칙은 P7을 따른다.

---

## III. 코딩 컨벤션

- 언어(응답·커밋·주석)·패키지 매니저(pnpm/uv)·테스트(vitest/pytest) 컨벤션은 [CLAUDE.md](../CLAUDE.md)를 단일 출처로 따른다
...
| 1.1.0 | 2026-05-22 | P1 자기모순 해소·HOW(토글) 분리, P4 메커니즘 일반화(시스템 프롬프트→AI 강제), P7(진실원 단일·파생물 비영속) 승격, P2 단계키 `meta`→`title`(제명) 정정·data-model §4.1 단일 출처화, III장 컨벤션 CLAUDE.md 참조로 압축 |
